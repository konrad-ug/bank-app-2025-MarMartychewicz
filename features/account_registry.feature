...
Scenario: User is able to update name of already created account
    Given Account registry is empty
    And I create an account using name: "anna", last name: "kowalska", pesel: "82010112345"
    When I update "name" of account with pesel: "82010112345" to "ania"
    Then Account with pesel "82010112345" has "name" equal to "ania"

Scenario: Created account has all fields correctly set
    Given Account registry is empty
    When I create an account using name: "john", last name: "doe", pesel: "90010199999"
    Then Account with pesel "90010199999" exists in registry
...
Scenario: User can make incoming transfer
    Given Account registry is empty
    And I create an account using name: "adam", last name: "nowak", pesel: "88010122222"
    When I make "incoming" transfer of "500" to account with pesel "88010122222"
    Then Account with pesel "88010122222" exists in registry